...
class SquareServer(object):
    ...
    def start(self):
        ...
        try:
            ...
            print "Exception starting Squares!"
            traceback.print_exc()
    ...
    def stop(self):
        if self.running: # should be safe to call multiple times
            try:
                ...
                print "Exception stopping Squares!"
                traceback.print_exc()
    ...
    def go_web(self, app):
        ...
        cherrypy.quickstart(SquareWeb(app),
                '/',
                config=config)

class SquareWeb(object):
...
if __name__=='__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Squares Light Control')
    ...
    print "Using Square Simulator at %s:%d" % (sim_host, sim_port)

    from model.simulator import SimulatorModel
    model = SimulatorModel(sim_host, port=sim_port)
    full_squares = square.load_squares(model)

    app = SquareServer(full_squares, model, args)
    try:
        ...
        print "Unhandled exception running Squares!"
        traceback.print_exc()
    finally:
        app.stop()
