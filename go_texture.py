...
#  ChannelRunner
#  -------------
#  Holds 1 DMX or Processing runner and 1-or-2 SquareServers (that each contain their own Pixel model with Pixels)
...
#  SquareServer
#  ---------
#  Mixes a ShowRunner and a Pixel model to send show commands on to the hex model
#  Holds 1 ShowRunner and 1 Pixel model (containing Pixels)
#
#  ShowRunner
#  ----------
#  Contains 1 Pixel model
#  Get frames from the Python shows
#  Puts those frames on to Pixel Model
#
#  Pixel model
#  ---------
#  Dictionary of {coord: pixel}
#  Ability to set a pixel's color, clear all pixels, etc.
#  Each channel should have its own Pixel model
#  A Pixel model has a dictionary of Pixels = { coord: Pixel object }
#
#  Pixel
#  -----
...
FADE_TIME = 60  # Fade In + Out times in seconds. If FADE_TIME == SHOW_TIME, then "always be fading"
...
class ChannelRunner(object):
    ...
    def go(self):
        """Run a micro-frame:
           Interpolates all pixels between the pixel_model's current color & next frame color as fast as possible
        ...
        if not self.one_channel:
            ...
            channel1_pixel_model, channel2_pixel_model = self.channels[0].pixel_model, self.channels[1].pixel_model

            # Two Channels require channel interpolation
            for coord in channel1_pixel_model.all_cells():
                pixel1, pixel2 = channel1_pixel_model.get_pixel(coord), channel2_pixel_model.get_pixel(coord)
...
class SquareServer(object):
    """Two channels = two different SquareServers
       Mixes a ShowRunner with a pixel model to send show commands on to the hex model
       Holds 1 ShowRunner and 1 pixel model (containing Pixels)"""
    ...
    def stop(self):
        if self.running:  # should be safe to call multiple times
            try:
                ...
                print("Exception stopping LEDs! {}".format(e))
                traceback.print_exc()
...
class ShowRunner(threading.Thread):
    ...
    def __init__(self, model, channel=0, one_channel=True, max_show_time=60):
        ...
        self.model = model  # pixel_model class within square.py
        self.running = True
        self.max_show_time = max_show_time
        self.show_runtime = 0
...
if __name__ == '__main__':
    """Main call function for the LEDs"""
    import argparse

    parser = argparse.ArgumentParser(description='Light Control')
