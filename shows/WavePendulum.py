	...
	def next_frame(self):
		...
		while (True):
			...
			for x in range(self.square.width):
				...
				self.square.set_cell((x, y), changeColor(self.color, y * self.gradient / 1500.0))
			...
			if self.counter % (int(2 * pi * self.cycles)) == 0:
				...
				self.color = changeColor(self.color, 0.15)
				self.background = randColorRange(self.background, 0.015)
